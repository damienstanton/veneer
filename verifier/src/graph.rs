...
/// Current on-disk graph format version. Bump on any incompatible wire-shape
/// change; `load` treats any other (or absent) version as "never built".
...
pub fn store(root: &Path, g: &Graph) -> std::io::Result<()> {
    ...
    let od = OnDisk { version: GRAPH_VERSION, entries, built_from: g.built_from, hash };
...
/// Load the graph. The graph is a cache with its own witness, orthogonal to
/// the ship gate — so corruption is absence, not an error: a missing file,
/// a malformed/old-format document, a version other than GRAPH_VERSION, or
/// a content-hash mismatch all read as the empty default ("never built"),
/// stale against any tree, regenerated by the next clean check or
/// `graph build`. Only a real IO error (permission denied, etc.) surfaces
/// as a Protocol finding — that is a problem with the machine, not the
/// cache, and regeneration cannot fix it.
    ...
    let raw = match std::fs::read_to_string(graph_path(root)) {
        ...
        Err(e) => {
            return Err(Finding::error(
                ...
                ".veneer/graph.toon",
                None,
                &format!("cannot read graph file: {e}"),
                Some("check permissions on .veneer/graph.toon"),
            ))
        }
    };
    let Ok(od) = toon_rust::from_str::<OnDisk>(&raw) else { return Ok(Graph::default()) };
    if od.version != GRAPH_VERSION {
