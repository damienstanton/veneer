...
subset on the wire); the logical state, the integrity hash, and all JSON
output carry normal UTF-8.
...
is never read by `check`, never gates a transition; a missing or
stale graph is a query-time concern only. The cache self-heals: a malformed,
old-format (`version` ≠ current), or hash-mismatched file reads as the empty
"never built" graph — stale, regenerated by the next clean check — never a
finding. Only a real IO error (e.g. permission denied) is a Protocol finding.
Building/rebuilding it is idempotent: an
